 ...
 * - "Little and often": ~50-65mg every 2-2.5 hours
 * - Hard cutoff: no caffeine within 8 hours of planned sleep
 * - Caffeine half-life: 3-5 hours
 *
 * Dry Brew = 65mg caffeine per serving
 */

const DRY_BREW_MG = 65;
const CAFFEINE_CUTOFF_HOURS = 8;
const DOSE_INTERVAL_HOURS = 2.5; // 65mg is slightly above 50mg target, so space slightly wider
...
function generatePlan(flight, opts = {}) {
  ...
  // Summary
  const totalDryBrews = days.reduce((sum, day) => {
  ...
  return {
    summary: {
      ...
      totalDryBrews,
      totalCaffeineMg: totalDryBrews * DRY_BREW_MG,
      flightDuration: `${Math.floor(flightHours)}h ${Math.round((flightHours % 1) * 60)}m`,
