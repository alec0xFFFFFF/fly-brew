...
async function lookupFlight(flightNumber, date) {
  ...
  // Free tier doesn't support flight_date — fetch by flight number only
  const url = `${API_BASE}/flights?access_key=${encodeURIComponent(apiKey)}&flight_iata=${encodeURIComponent(iata)}`;
  ...
  if (!json.data || json.data.length === 0) {
    throw new Error(`No flight found for ${iata}`);
  }
